            ...
            for j in range(0, offspring.shape[1] - len(list_of_functions)):
                if (i1 != i2):
                    ...
                    if (rand <= 0.5):
                        ...
                        b_offspring = b_offspring**(1/(mu + 1))       
                    offspring[count,j] = np.clip(((1 + b_offspring)*archive[i][parent_1][j] + (1 - b_offspring)*archive[i][parent_2][j] )/2, min_values[j], max_values[j])           
...
def neighborhood_sensitive_archived_evolutionary_many_objective_optimization(references = 5, min_values = [-5,-5], max_values = [5,5], list_of_functions = [func_1, func_2], generations = 50, theta = 5, k = 4):       
    count      = 0
